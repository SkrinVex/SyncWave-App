...
class SetupAdminUseCase(private val authRepository: AuthRepository) {
    suspend operator fun invoke(username: String, password: String): Resource<AuthSession> {
        if (username.isBlank()) return Resource.Error("Введите имя администратора")
        ...
        return authRepository.setupAdmin(username.trim(), password)
    }
}
